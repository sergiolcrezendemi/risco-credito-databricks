...
def run_bronze_ingestion(
    ...
    if row_count == 0:
        raise RuntimeError(
            f"[FALHA] {target_table} está vazia após a ingestão. "
            f"Causas comuns: (1) nenhum .csv em {raw_data_path}, "
            f"(2) checkpoint em {checkpoint_path} já marcou os arquivos como "
            f"processados de uma execução anterior sem dados — nesse caso, "
