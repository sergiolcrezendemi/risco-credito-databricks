...
|---|---|---|
| `03_inferencia_batch.py` | `src/risco_credito/ml/decisao.py` — `classificar_decisao()` | `tests/test_decisao.py` (6 testes: limites do threshold, intervalo [0,1] inválido) |
| `01_drift_dados.py` | `src/risco_credito/monitoring/data_drift.py` — `calcular_psi()`, `classificar_psi()` | `tests/test_data_drift.py` (10 testes: PSI zero em distribuições idênticas, cresce com o deslocamento, ignora NaN, limiares moderado/severo) |
| `02_concept_drift.py` | `src/risco_credito/monitoring/concept_drift.py` — `avaliar_queda_auc()`, `gerar_lote_sintetico()` | `tests/test_concept_drift.py` (7 testes, incluindo um de integração: treina uma Regressão Logística na referência e confirma que a AUC cai no lote com concept drift simulado) |
